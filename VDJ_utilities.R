...
trace.VDJ.duplicates <- function(seurat.obj, 
  ...
  input.vdj <- input.vdj %>% arrange(chain, cdr3, productive, reads, v_gene, j_gene, d_gene, c_gene) %>%
    filter(cdr3 != 'None' & cdr3 != '' & vdj_source == '10X') %>% 
  ...
  barcode.counts.prod <- input.vdj %>% group_by(chain, barcode) %>%
    filter(grepl('[T|t]rue', productive)) %>% summarise(counts = n())
  
  # get all cells that have TCR sequences over given limits
  multiple.A <- barcode.counts %>% filter(chain == 'TRA', counts > lim.A.all) %>% 
    pull(barcode)
  multiple.B <- barcode.counts %>% filter(chain == 'TRB', counts > lim.B.all) %>% 
    pull(barcode)
  multiple.A.prod <- barcode.counts.prod %>% filter(chain == 'TRA', counts > lim.A.prod) %>% 
    pull(barcode)
  multiple.B.prod <- barcode.counts.prod %>% filter(chain == 'TRB', counts > lim.B.prod) %>% 
    pull(barcode)
...
create.VDJ.metadata <- function(seurat.obj, 
  ...
  if(filter.vdj){
    ...
    # get all cells that have TCR sequences over given limits
    multiple.A <- barcode.counts %>% filter(chain == 'TRA', counts > 2) %>% 
      pull(barcode)
    multiple.B <- barcode.counts %>% filter(chain == 'TRB', counts > 2) %>% 
      pull(barcode)
    multiple.A.prod <- barcode.counts.prod %>% filter(chain == 'TRA', counts > 2) %>% 
      pull(barcode)
    multiple.B.prod <- barcode.counts.prod %>% filter(chain == 'TRB', counts > 1) %>% 
      pull(barcode)
    
    multiple.chain <- unique(c(multiple.A, multiple.B, multiple.A.prod, multiple.B.prod))
    multiple.chain <- intersect(multiple.chain, colnames(seurat.obj))
    input.vdj <- input.vdj %>% filter(!(barcode %in% multiple.chain))
  }
  
  # remove all non-productive VDJs
  input.vdj <- input.vdj %>% filter(grepl('[T|t]rue', productive)) %>% 
    arrange(chain, vdj_source, cdr3, desc(reads), v_gene, j_gene, d_gene, c_gene)
  
  # reformat data
  # first we'll treat columns we can't/don't wan't to just aggregate
  # Multi results will be aggregated
  vdj.data <- input.vdj %>% select(barcode, chain, v_gene, d_gene, j_gene, cdr3, cdr3_nt)
  vdj.summary <- vdj.data %>% group_by(barcode, chain) %>% summarise(count = n())
  vdj.summary.A <- vdj.summary %>% filter(chain == 'TRA') %>% select(-chain) %>%
    dplyr::rename(count_TRA = count)
  vdj.summary.B <- vdj.summary %>% filter(chain == 'TRB') %>% select(-chain) %>% 
    dplyr::rename(count_TRB = count)
  vdj.summary.Multi <- vdj.summary %>% filter(chain == 'Multi') %>% select(-chain) %>%
    dplyr::rename(count_Multi = count)
  vdj.data.A1 <- vdj.data %>% group_by(barcode) %>%
    filter(chain == 'TRA') %>%  filter(row_number()==1) %>%
    select(-d_gene, -chain) %>%
    dplyr::rename(cdr3_A1 = cdr3, cdr3_A1_nt = cdr3_nt, 
                  v_gene_A1 = v_gene, j_gene_A1 = j_gene)
  vdj.data.A2 <- vdj.data %>% group_by(barcode) %>%
    filter(chain == 'TRA') %>% filter(row_number()==2) %>%
    select(-d_gene, -chain) %>% 
    dplyr::rename(cdr3_A2 = cdr3, cdr3_A2_nt = cdr3_nt, 
                  v_gene_A2= v_gene, j_gene_A2 = j_gene)
  vdj.data.B <- vdj.data %>% group_by(barcode) %>%
    filter(chain == 'TRB') %>% filter(row_number()==1) %>%
    select(-chain) %>% 
  ...
  VDJ.cell.descr <- input.vdj %>% select(barcode, is_cell) %>% 
    group_by(barcode) %>% filter(row_number()==1)
  ...
  VDJ.TCR.descr <- input.vdj %>% 
    ...
    filter((row_number() <= 2 & chain == 'TRA') | (row_number() <= 1 & chain == 'TRB') | 
           (row_number() <= 1 & chain == 'Multi')) %>% ungroup %>%
    select(barcode, chain, contig_id, high_confidence, productive, length, reads, umis, vdj_source) %>%
    group_by(barcode) %>% 
  ...
  VDJ.meta.data <- VDJ.cell.descr %>% left_join(VDJ.TCR.descr, by = "barcode") %>% 
    ungroup %>% arrange(barcode) %>% left_join(vdj.summary.A, by = "barcode") %>%
  ...
  VDJ.meta.data <- select(VDJ.meta.data, is_cell, chain, contig_id, high_confidence, productive, length, 
...
get.sequence.clonotype <- function(input.vdj){
  clono.numbering <- input.vdj %>% 
    select('chain', 'cdr3') %>% group_by(chain, cdr3) %>% 
    filter(chain == 'TRA' | chain == 'TRB') %>%
    filter(row_number()==1) %>% ungroup %>% group_by(chain) %>%
...
get.clonotypes <- function(seurat.obj, clono.numbering){
  ...
  clono.numbering.A <- clono.numbering %>% filter(chain == 'TRA') %>%
    ungroup %>% select(cdr3, clonotype) 
  clono.numbering.B <- clono.numbering %>% filter(chain == 'TRB') %>%
    ungroup %>% select(cdr3, clonotype)
  

  # associate cells and clonotype tags
  meta.data.A1 <- meta.data.table %>% select(cdr3_A1) %>%
    left_join(clono.numbering.A, by = c('cdr3_A1' = 'cdr3'))
  meta.data.A2 <- meta.data.table %>% select(cdr3_A2) %>%
    left_join(clono.numbering.A, by = c('cdr3_A2' = 'cdr3')) 
  meta.data.B <- meta.data.table %>% select(cdr3_B) %>%
  ...
  # fill clonogroup assay, insert it to the seurat object
  clonogroups <- table(unlist(clonotypes))
  clonogroups <- clonogroups[clonogroups > 1]
  
  clonogroup.assay <- matrix(0, nrow = length(clonogroups), ncol = nrow(meta.data.table) )
  colnames(clonogroup.assay) <- rownames(meta.data.table)
  rownames(clonogroup.assay) <- names(clonogroups)
  
  pbar <- pbapply::timerProgressBar(min=0, max=nrow(seurat.obj@meta.data),
                                    width = 50, style = 6, char = '|+ |')
  for(i in 1:nrow(seurat.obj@meta.data)){
    for(j in 1:length(seurat.obj@meta.data$clonotypes[[i]])){
      if(seurat.obj@meta.data$clonotypes[[i]][j] %in% rownames(clonogroup.assay) ){
        clonogroup.assay[seurat.obj@meta.data$clonotypes[[i]][j], 
                         rownames(seurat.obj@meta.data)[i]] <- 1
      }
    }
    pbapply::setTimerProgressBar(pbar, i)
  }
  close(pbar)
  
  seurat.obj[['Clonogroups']] <- CreateAssayObject(clonogroup.assay)
...
create.clones <- function(seurat.obj){
  ...
  cdr3.table <- seurat.obj@meta.data %>% 
    select(cdr3_A1, cdr3_A2, cdr3_B)
  for(i in 1:nrow(cdr3.table)){
    cdr3.table[i,1:2] <- cdr3.table[i,order(cdr3.table[i,1:2])]
  }
  ...
  double.A.clones.multi <- double.A.clones %>% filter(occurences > 1)
  ...
  for(i in 1:nrow(single.A.clones)){
    ...
    if(length(singular.clones) == 1){
      ...
      # 2.) yes: check if it is compatible with single CDR3A and only then add CDR3A with given clone
    ...
    # similar to above; it is enough if only one is not annotated or compatible
    added <- 0
    for(k in 1:length(singular.clones)){
      if(!(clonodict[singular.clones[k]] %in% names(clonodict))){
        added <- added + 1
        clonodict[singular.clones[1]] <- single.A.clones$tmp.clone[i] 
      }
    }
    if(added > 0){ # add immediately if at leats one clonotype was added previously
      clonodict[single.A.clones$tmp.clone[i]] <- single.A.clones$tmp.clone[i]
    }else if(added == 0){ # if not check for compatibility with existing clonotypes
      for(k in 1:length(singular.clones)){
  ...
  single.A.clones <- single.A.clones %>% 
    filter(occurences > 1 & !(tmp.clone %in% clonodict.names))
...
insert.missing <- function(vdj,
  ...
  vdj.fill <- vdj %>% 
    filter(chain == missing.chain & cdr3 == CDR3_1) %>%
    filter(row_number() == 1) %>%
